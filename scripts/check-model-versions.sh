...
# `<!-- CURRENT: ... -->` marker in .agents/references/model-versions.md.
#
...
SSOT="$REPO/.agents/references/model-versions.md"
if [ ! -f "$SSOT" ]; then
...
if [ "$drift" -ne 0 ]; then
    ...
    echo "Source of truth: .agents/references/model-versions.md" >&2
    exit 1
fi
