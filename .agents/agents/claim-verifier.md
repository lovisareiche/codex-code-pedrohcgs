...
that reason rather than reporting a match. See `.agents/rules/review-fencing.md`.
...
- `.agents/rules/post-flight-verification.md` — the protocol callers follow.
- `.agents/rules/review-fencing.md` — independence as a property of the environment; the dispatcher's duty to fence a pinned value before a reproduce-a-pinned-number task.
- `.agents/skills/verify-claims/SKILL.md` — user-facing wrapper.
