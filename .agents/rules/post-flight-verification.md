...
Symmetric partner to **Pre-Flight Reports** (`.agents/rules/content-invariants.md` + skill-level `## Phase 0`). Where Pre-Flight proves inputs were read *before* work, Post-Flight proves the output's factual claims hold *after* drafting — before the skill returns to the user.
...
- `.agents/rules/review-fencing.md` — the environment side of the same discipline: a forked context does not fence the checkout the reviewer stands in.
- `.agents/skills/verify-claims/SKILL.md` — user-facing wrapper for ad-hoc verification of any text.
- `.agents/rules/content-invariants.md` — Pre-Flight (input side).
- `.agents/rules/cross-artifact-review.md` — pattern-based; Post-Flight is draft-based.
- `.agents/rules/summary-parity.md` — rule against enumerative summaries drifting from their bodies; Post-Flight is the factual equivalent for draft content.
