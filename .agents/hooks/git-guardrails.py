    ...
    - git merge / rebase / pull unless the invocation is a STANDALONE SIMPLE
      ...
      Rules 0-3 below are what happens to an op that IS identified.
        ...
        3. Otherwise READ THE TREE, live: `git status --porcelain` in the
           ...
           shipped against a 5s `"timeout"` in .agents/settings.json, so on the
...
# `rm -f .agents/hooks/git-guardrails.py` — every one of which DENIES when
...
# `.agents/settings.json`. With line 1 = an ordinary comment carrying no `<<`,
    ...
    for seg in _segments(_join_continuations(_strip_heredocs(cmd))):
        ...
        for name, predicate, reason, alt in GIT_DENY:
            if sub == name and predicate(args):
                return (f"{reason} {alt} "
                        f"(override: run it in a terminal yourself, outside Codex.)")
    return None
...
#     timeout in .agents/settings.json, with margin for the rest of the hook's
...
# .agents/settings.json registers, and `_status_timeout()` CLAMPS rather than
