# Application: auditing this repository's own infrastructure

The general method in `SKILL.md` applied to the template itself — skills, agents, rules,
hooks, scripts, and docs. Read this when the audit target is the repo, not a paper.
...
Focus: **all** executable code in the repo — `.agents/hooks/*.py`, `.agents/hooks/*.sh`, `scripts/*.py`, `scripts/*.sh`, `.agents/scripts/*.sh`. Not just `.agents/hooks/` — when PR #93 added new code under `scripts/`, the original narrow scope meant Copilot + Codex caught 5 bugs the audit missed.

Hook-specific checks (Stop/PreToolUse/SessionStart protocols, `CLAUDE_PROJECT_DIR` usage, hash-length consistency) apply only to `.agents/hooks/`. Everything below applies to ALL executable code:
...
Focus: `.agents/skills/*/SKILL.md` and `.agents/rules/*.md`
...
- All templates referenced in `.agents/rules/*.md` and the guide (`guide/workflow-guide.qmd`) exist in `templates/`
...
- `allowed-tools` linter warning — known linter bug (Codex issue #25380), field IS valid
...
- `log-reminder.py` outputting `{"decision": "block"}` with `sys.exit(0)` — this IS the modern Codex Stop-hook block protocol, NOT a bug
