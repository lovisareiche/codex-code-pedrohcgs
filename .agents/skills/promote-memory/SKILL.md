...
<!-- Pattern adapted with attribution from Chris Blattman's claudeblattman v2.1
     ...
     .agents/rules/meta-governance.md. -->
...
- **native auto memory** (`~/.agents/projects/<project>/memory/` — machine-local, typed `user`/`feedback`/`project`/`reference`, no size cap on topic files) — machine-specific and user-specific learnings.
...
> "Does the entry follow the schema in [`.agents/rules/meta-governance.md`](../../rules/meta-governance.md): `[LEARN:category] wrong → right` for corrections, structured `**Why:**` + `**How to apply:**` for feedback/project entries? If it's just a free-form note, vote NO — fix the format first, then re-submit."
...
If `$ARGUMENTS` is `all`, read every `[LEARN:*]` entry in `~/.agents/projects/<project>/memory/`. Otherwise treat `$ARGUMENTS` as a substring filter (e.g., `r-code` matches all `[LEARN:r-code]` entries).
...
- **Format critic** — context: the candidate entry + [`.agents/rules/meta-governance.md`](../../rules/meta-governance.md) for the schema reference.

Use the **Haiku tier** for all five critics (per [`.agents/rules/model-routing.md`](../../rules/model-routing.md): mechanical-ish review work). The user can override via the agent's `model:` field if they want Sonnet for the harder calls.
...
- [`.agents/rules/meta-governance.md`](../../rules/meta-governance.md) — the two-tier memory contract this skill operationalizes.
- [`.agents/agents/promote-memory-council.md`](../../agents/promote-memory-council.md) — the five-critic implementation (one agent file with five role specs, dispatched in parallel via the `Agent` tool).
- [`.agents/rules/model-routing.md`](../../rules/model-routing.md) — why critics default to Haiku tier.
...
Candidates come from **native auto memory** — `~/.agents/projects/<project>/memory/`. Claude
...
**Retired:** `.agents/state/personal-memory.md`. The two-tier idea was right; Codex now
