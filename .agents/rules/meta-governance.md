...
> That is **not** how this file loads: `AGENTS.md` only markdown-links `MEMORY.md`, it does
...
> documented behaviour, add `@MEMORY.md` to `AGENTS.md` — but note the file is already
...
**Where it lives now:** Codex ships this natively. It is **on by default** and writes to
`~/.agents/projects/<project>/memory/` — a `MEMORY.md` index plus one topic file per memory,
...
hand-rolled plumbing is obsolete. `.agents/state/personal-memory.md` is **retired** — if you
...
| Machine-specific setup | ❌ No | native auto memory (`~/.agents/projects/<project>/memory/`) | ❌ No (machine-local) |
| Templates (generic) | ✅ Yes | templates/ | ✅ Yes |
| Skills (generic) | ✅ Yes | .agents/skills/ | ✅ Yes |
| Rules (path-scoped, generic) | ✅ Yes | .agents/rules/ | ✅ Yes |
