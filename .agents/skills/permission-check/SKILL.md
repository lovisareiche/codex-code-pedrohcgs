...
Surface the full permission-mode picture across every layer Codex honors, so the user can see at a glance why prompts are (or aren't) firing. Codex resolves permission mode from a 6-tier stack; a single misconfigured layer produces silent overrides that are hard to debug by eye.
...
The live in-session mode is exposed via the status line (see `.agents/scripts/statusline.sh`). Tell the user:

> "Your status line shows the current in-session mode in the top-right of the Codex panel. If that disagrees with the resolved `defaultMode` above, you (or Shift+Tab) overrode it mid-session. Press Shift+Tab to cycle back."

If the status line isn't configured, emit a warning and point at `.agents/scripts/statusline.sh`.
...
2. **VSCode-only bypass:** VSCode layers say bypass but no CLI layer does → terminal Codex will still prompt; extension may or may not.
3. **Empty allowlist + default mode:** `defaultMode: "default"` with empty `allow` → every tool prompts, as designed.
4. **Stale session:** settings are correct but user reports prompts → almost always a session that pre-dates the fix. Advise "Cmd+Shift+P → Developer: Reload Window, then new Codex session."
...
Check the status line at the top of the Codex panel. Expected: [BYPASS].
