...
**Last reviewed:** 2026-08-22 (v2.5.0 release sweep)
...
- ✅ **Stata expansion** — `/stata-replication` + `.agents/rules/stata-code-conventions.md` + `/audit-reproducibility` Stata coverage shipped (Pass 4). Was not originally on the v1.9 backlog but added per user request mid-cycle.
...
- ✅ **All five long-deferred capability skills** — `/syllabus`, `/teach-from-paper`, `/power-analysis`, `/replication-package`, `/respond-to-eval` (deferred since the v1.9.0 plan) all shipped, plus `/capture-environment`, `did-event-study` (since removed, 2026-08-22), `/disclosure-check`, `/grant-proposal`, `/data-management-plan`, `/coauthor-brief`, `/scaffold-exercises`, `/new-skill`, and `/triage-inbox`.
...
- **Non-Claude coauthor gate note** — document the path for a coauthor who pulls the repo *without* Codex: `./scripts/install-hooks.sh` works for them too (pre-commit is plain bash/python), but skills/hooks guidance assumes the Claude loop; a short "for your non-Claude coauthor" README box would close it.
...
Also surfaced by the audit; lower priority and a scope decision (centralize here vs. keep in dedicated private repos with only an MCP bridge here).
...
Considered for v1.10.0 and pulled before ship: a `.claude-plugin/marketplace.json` exposing `review-suite` / `simulation-suite` as installable subsets. **UNBLOCKED 2026-08-21:** [#11278](https://github.com/anthropics/claude-code/issues/11278) **closed 2025-11-09** — the path-resolution bug this deferral rested on is fixed. The à-la-carte packaging decision is now a scope call, not a technical blocker. Re-scoped in the v2.5 plan: packaging is now a **scope decision**, no longer a technical blocker. (Historical: v2.0 re-affirmed the non-goal while #11278 was still open.)
...
`/syllabus`, `/teach-from-paper`, `/power-analysis`, `/replication-package`, and `/respond-to-eval` — deferred since the v1.9.0 plan — **all shipped in v2.0.0**, alongside `/capture-environment`, `did-event-study` (since removed, 2026-08-22), `/disclosure-check`, `/grant-proposal`, `/data-management-plan`, `/coauthor-brief`, and `/scaffold-exercises`. See the "Shipped in v2.0.0" section above and CHANGELOG.md.

### Mechanical / discipline upgrades

- ✅ **SHIPPED v2.5 as `/voice-profile`.** ~~Personal-style-guide reference (positive direction)~~ — voice extraction template. User points at 2-3 prior papers; Claude extracts lexicon, sentence rhythm, intro patterns. Used by `/respond-to-referees`, `/research-ideation` rationale sections, and any draft-side work to keep voice consistent across an author's corpus. **Complement to `/humanize`** (which is the *negative* direction — detect AI voice). Together: detect AI tells (negative) + audit against your own profile (positive). Output: `style-profile.md` referenced by `humanize-auditor` to respect documented authorial preferences (e.g., "I use em-dashes deliberately").
- ✅ **SHIPPED v2.0.0** (`check-surface-sync.py` TABLE-ROW assertions for marked tables; unmarked tables like the guide appendix still need semantic sweeps). ~~Enumerative-table consistency check~~ — add a mechanical check (or extend `check-surface-sync.py`) that counts `.agents/{skills,agents,rules}/*` and verifies appendix table row counts match. Pet-peeve #18 documents the class. **Promoted to high-priority** after v1.9.0 added 6 skills + 2 agents; manual appendix-table updates were a meaningful share of every PR. A mechanical check would have caught the pre-existing v1.5.0 peer-review-trio drift in 1 second instead of 3 releases.
...
- ✅ **DONE v2.5.0 (2026-08-23) — auto-mode messaging normalized** across README, guide, and TROUBLESHOOTING; the shipped bypass default stays per the owner's ruling. ~~Auto mode messaging — NOW DUE (2026-08-21).~~ Auto mode became the **default** starting mode for new Pro/Max/Team sessions on 2026-08-14, so the template's shipped `bypassPermissions` default now diverges from the platform default that forkers will otherwise get. The owner's own bypass preference stands for their machine; the *template's advice* to forkers should follow the platform. Per `feedback_bypass_permissions.md` memory, the *shipped* `defaultMode` stays bypass; this is a docs-only change. v1.9.0 noted auto mode no longer needs the `--enable-auto-mode` flag (now on Team / Enterprise / API and rolling out to Max; needs Opus 4.6+ or Sonnet 4.6); further normalising the messaging is the v2.0 follow-up.

### From the v2.5.0 external Oracle review (2026-08-22)

Adopted in v2.5.0: adoption ladder, enforcement matrix, pattern pathways, red/green demo,
git survival kit, license decision steps, claims legend, typed tolerances, version-parity +
injection-syntax staleness checks. The rest, dispositioned per owner rulings:

- **Replayable qualification fixtures** (`qualification-fixtures/` with per-defect branches, prompts, replay script) — deferred; the ledger records results, fixtures would make them replayable.
- **Interface/version feature-support matrix** — deferred; maintenance cost vs. the per-surface caveats now inline.
- **Complete empirical-paper walkthrough + worked Monte Carlo chapter** — **blocked by owner ruling (2026-08-22): no prescriptive empirical-practice content without explicit sign-off of the exact text.** Do not build without an owner ask.
- **UNVERIFIED blocks commits** — **rejected by owner ruling (2026-08-22): no additional session-time enforcement; honest UNVERIFIED reporting stays, blocking does not ship.**
...
- ~~**`/prompt-refine`**~~ — retired with the `/prompt` pair in v2.0 (the shaping habit replaced the skill family); kept for the record, not a live item.
...
- **Cost-Conscious Composition telemetry** — ✅ per-agent pinning shipped in v2.0.0 (all 18 agents carry explicit `model:` + `effort:`). Remaining: measure the actual cost impact of the 70/20/10 routing against an unpinned baseline.
