...
**Fable 5** (GA 2026-06-09) is the most capable model in Codex — and this rule deliberately does **not** route any of the template's fleet to it. Two verified reasons:
...
- [`.agents/rules/cross-artifact-review.md`](cross-artifact-review.md) — paper ↔ code dependency graph (orthogonal to routing but invoked at similar moments).
- [`.agents/rules/post-flight-verification.md`](post-flight-verification.md) — CoVe / forked verifier (claim-verifier should stay on Opus per "anti-pattern: pushing Opus down" above).
