...
- **`MEMORY.md` syncs via git; native auto memory (`~/.agents/projects/<project>/memory/`) stays machine-local** (`meta-governance.md`). Machine-specific restricted-data *paths* live in personal-memory, never in committed files.
...
- [`.agents/rules/replication-protocol.md`](replication-protocol.md) — passport entries note disclosure status.
- [`.agents/rules/meta-governance.md`](meta-governance.md) — the committed-vs-local two-tier model this builds on.
