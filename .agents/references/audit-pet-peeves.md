...
**How to use.** When writing a deep-audit agent prompt, say "read `.agents/references/audit-pet-peeves.md` and explicitly check for each class before reporting clean." When triaging a Copilot/Codex finding, decide: is this a new class (add entry) or an existing class re-surfacing (bump the evidence list)?
...
**How to catch.** Not mechanical — it's a judgment pattern. The rule `.agents/rules/summary-parity.md` encodes it: two review-bot flags on the same summary paragraph = rewrite structurally (abstract up), don't patch.
