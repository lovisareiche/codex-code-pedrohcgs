...
none of them trips this hook:
    ...
    1. an Edit / Write / MultiEdit on these same files (deliberately allowed —
       ...
       from `.agents/settings.json`. The diff is visible AFTERWARDS; under
    ...
    2. an ordinary branch transition — `git switch <branch>` / `git checkout
       ...
       `.agents/hooks/` or carries an older `.agents/settings.json`. There is
...
decide whether any other gate runs at all:
    ...
    .agents/settings.json        — which hooks fire, and on what
    .agents/settings.local.json  — the same, per machine
    .agents/hooks/               — the gates themselves
    .githooks/                   — the pre-commit gate suite
    .agents/                     — the directory, because deleting it
                                   deletes the hooks
...
second checkout, and the user's own `~/.agents/settings.json` are other
...
Denied, when the target is a protected path:
    ...
    - command wrapper        env / sudo / doas / nice / stdbuf / ionice /
                             ...
                             `-c` OR a bare deleter (`nice rm .agents/...`) — is
...
protective control elsewhere at all:
  ...
  - a machine-wide guard in `~/.agents/hooks/` is OPTIONAL and lives outside
...
shell GLOB or a BRACE GROUP is caught since r15 (`rm -f .agents/hook?/…`,
`.agents/*/…`, `.clau*/hooks/…`, `settings.jso?`, `hook[s]`, `{hooks,rules}`,
...
another name, it truncated `.agents/settings.json` in a throwaway fixture while
...
BOUNDARY, not as a defect:
  ...
  - a protected path produced by a COMMAND SUBSTITUTION rather than written as
    a literal — `echo X > $(echo .agents/settings.json)`, and the same write
  ...
  - a VARIABLE OR SUBSTITUTION SITTING INSIDE A PROTECTED SEGMENT'S NAME, which
    ...
    project fixture: `rm -f $UNSET/.agents/hooks/…` DENIES, exactly as that
  ...
  - a write ROUTED THROUGH A SYMLINK whose own name is not protected. The
    ...
    throwaway project fixture, with `linkhooks -> .agents/hooks` and
    `linkdot -> .claude`: `rm -f linkhooks/git-guardrails.py`,
    `rm -f linkdot/hooks/git-guardrails.py`, `rm -f .agents/linkin/…` (the link
    inside `.claude` itself) and `echo X > linkdot/settings.json` were all
    ALLOWED (silent), while the literal twin `rm -f .agents/hooks/…` DENIED.
  ...
  - a `cd` EARLIER IN THE SAME COMMAND LINE. Relative tokens resolve against
    ...
    `cd /tmp/fixture && printf '{}' > .agents/settings.json` is scored against
  ...
  - a path built from a variable this process cannot expand (`$FIXTURE/.agents/
  ...
  - an UNKNOWN wrapper or execution form not in the WRAPPERS/SHELLS tables, or
    ...
    fixture, `nice rm -f .agents/hooks/git-guardrails.py` DENIED while
  ...
  - a GLOB whose expansion depends on a shell OPTION this scan does not model
    ...
    `> .agents/settings.jso?` case, added filesystem work to every command
...
that launches Codex). Prefixing the blocked Bash command with it does
...
# Children of `.agents/` that are gate-defining. Everything else under
# `.agents/` (rules, skills, references, agents) is ordinary content.
...
def normalize(path: str) -> str:
    """Collapse separator noise so `.agents//hooks` and `.agents/./hooks`
    match the same rule as `.agents/hooks`.
    ...
    before it), so `.CLAUDE/settings.json` and `.agents/settings.json` are ONE
...
# shell really does:
#
#     rm -f .agents/rules/../hooks/git-guardrails.py   SILENT -> file deleted
#     echo x > .agents/rules/../settings.json          SILENT -> file rewritten
#     rm -f .agents/agents/../settings.local.json      SILENT
#
...
# unprotected child of `.agents/` works as the pivot (rules, agents, skills,
# references, state); `.agents/../.agents/hooks/x` already denied, because
...
# exist at hook time, and `> .agents/rules/../settings.jso?` names a file that
...
# `.agents/hooks` must keep being judged as naming it. The union can only add
...
def resolve_parents(path: str) -> str:
    ...
    A `..` pops the segment before it (`.agents/rules/..` -> `.claude`). Two
    edge cases decide the rest:

      * a LEADING `..`, or one that would climb above the start of a RELATIVE
        token, has nothing to pop and is KEPT (`../.agents/hooks/x` stays
        `../.agents/hooks/x`, which still names the protected directory);
        under an ABSOLUTE token it is dropped, because POSIX defines `/..`
        as `/`.
      * a TRAILING `..` is an ordinary pop (`.agents/hooks/..` -> `.claude`,
...
# does not exist YET, which is exactly the `> .agents/settings.jso?` case.
#
...
# the ordinary names under `.agents/` (`hooks`, `settings.json`,
# `settings.local.json`) a bare wildcard matches — because there it really
# does expand: `rm -rf .agents/*` deletes the hooks. `dotglob` being set is
...
def _brace_expand(token: str) -> list[str]:
    """Expand shell BRACE GROUPS: `.agents/{hooks,rules}/x` -> two tokens.
...
# --- project scope ---------------------------------------------------------
#
# The pattern above is a TEXT match, so on its own it protects `.agents/hooks`
...
# fixture clone's `.agents/settings.json`, and it contradicted the deny message,
...
# Precedence: CLAUDE_PROJECT_DIR (what Codex sets for hooks) → the git
...
def in_project(token: str) -> bool:
    ...
    `$VAR` are expanded, so `$HOME/.agents/settings.json` is recognised as the
...
#
#     echo CLOBBERED >& .agents/settings.json        ALLOW -> file is 'CLOBBERED'
#     echo CLOBBERED >&.agents/settings.json         ALLOW -> file is 'CLOBBERED'
#     echo CLOBBERED >& .agents/hooks/git-guardrails.py  ALLOW -> guard overwritten
#
...
# above never named it — so `git rm .agents/hooks/x`, `git checkout HEAD~1 --
# .agents/settings.json`, `git restore .agents/settings.json`, `git clean -fd
# .agents/hooks`, `git stash push -- .agents/hooks/x` and `git mv` all
...
# a throwaway project fixture holding `.agents/settings.json` and
# `.agents/hooks/`:
#
...
# `    .agents/hooks/git-guardrails.py` emitted no decision, and the same string
...
# `.agents/hooks/git-guardrails.py`. Same class as the fast-path defect above —
...
# SILENT on `rm -f .agents/hooks/git-guardrails.py` and on
# `echo x > .agents/settings.json`, and the sibling hook went SILENT on
...
# `.agents/hooks/` and `.agents/settings.json`; a git fixture with one modified
# tracked file), line 2 carrying the real op. With line 1 =
# `# heredoc <<EOF` this guard went SILENT on
# `rm -f .agents/hooks/git-guardrails.py` and on a redirect into
# `.agents/settings.json`, and the sibling hook went SILENT on
...
def strip_heredocs(cmd: str) -> str:
    """Drop heredoc BODIES. `cat <<'EOF' > .agents/settings.json` still denies —
    the redirection is on the command line — but a body that merely *documents*
    `rm .agents/hooks/x` is prose, not a command, and must not trip a rule."""
    if "<<" not in cmd:
        return cmd
...
def _compose_dash_c(parts: list[str]) -> str | None:
    ...
    byte-equivalent `git -C .agents/hooks clean -fd` DENIED (a22) — measured.
...
def git_write_target(args: list[str]) -> tuple[str, str] | None:
    ...
    writes: `git -C .agents/hooks clean -fd` deletes hooks without ever naming
...
def scan_segment(seg: list[tuple[str, str]]) -> tuple[str, str] | None:
    ...
    fixture, `RM -f .agents/hooks/git-guardrails.py` was ALLOWED (silent) while
    ...
    throwaway project fixture: `nice rm -f .agents/hooks/git-guardrails.py`
    DENIED, while `NICE rm -f .agents/hooks/git-guardrails.py` and
    `BASH -c 'rm -f .agents/hooks/git-guardrails.py'` were ALLOWED (silent).
    ...
    if key in DEST_LAST:
        ...
        # `ln`/`rsync` that operand is the SOURCE — `ln -s .agents/hooks/x`
        # creates `./x` POINTING AT the hook, and `rsync .agents/hooks/` lists
...
# *inside* quotes — an echo or a runbook line that documents `> .agents/...` —
...
def _git_guardrails():
    ...
    `.agents/hooks/git-guardrails.py` replaced by `import sys; sys.exit(0)`, a
    write into `.agents/hooks/` returned rc=0 and said nothing — a SILENT ALLOW
...
_HOOK_REGISTERED_TIMEOUT = 20.0   # MUST equal .agents/settings.json's "timeout"
...
def _denoise(cmd: str) -> str:
    ...
    nested — `bash -c 'rm .clau\\de/hooks/x'` -> `bash -c rm .agents/hooks/x`.
...
def main() -> int:
    ...
    deny(
        ...
        f"{scope}"
        f"define every gate (.agents/settings.json, .agents/settings.local.json, "
        f".agents/hooks/, .githooks/). Permissions run in bypass mode here, so a "
