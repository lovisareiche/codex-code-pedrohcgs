...
- Satisfies [`tikz-prevention.md`](../../.agents/rules/tikz-prevention.md): explicit node dimensions (P1), coordinate map comment (P2), no `scale=` (P3), directional keyword on every edge label (P4).
...
4. **When in doubt, run the six-pass check.** See [`tikz-measurement.md`](../../.agents/rules/tikz-measurement.md) for formulas. The `/extract-tikz` and `/new-diagram` skills run the checks for you.
...
- Passes the P3 and P4 grep pre-checks in [`tikz-prevention.md`](../../.agents/rules/tikz-prevention.md) by construction.
