...
**The shipped harness is primary.** From a normal shell (NOT inside a Codex session —
nested headless calls hang), at the repo root:

```bash
./scripts/run-skill-eval.sh <skill> .agents/skills/<skill>/evals/cases [--replicates N]
```
