...
- Native auto memory (`~/.agents/projects/<project>/memory/`) and `.agents/state/` stay **local** — never expect a co-author to have yours (see meta-governance.md).
...
- [`.agents/rules/meta-governance.md`](../../rules/meta-governance.md) — the cross-machine memory model (what syncs, what stays local) this brief operationalizes for *people*.
...
- [`.agents/rules/confidential-data.md`](../../rules/confidential-data.md) — restricted-data handling; the data section summarizes its access steps without exposing data.
